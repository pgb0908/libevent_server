 ...
 */
        ...
        public:
            ...
            void enableTimer(const std::chrono::milliseconds max_ms,
                ...
                //ENVOY_LOG_MISC(trace, "enableTimer called on {} for {}ms, min is {}ms",static_cast<void*>(this), max_ms.count(), min_ms.count());
        ...
        TimerPtr ScaledRangeTimerManagerImpl::createTimer(ScaledTimerMinimum minimum, TimerCb callback) {
            return std::unique_ptr<RangeTimerImpl>(new RangeTimerImpl(minimum, callback, *this));
        }
        ...
        ScaledRangeTimerManagerImpl::activateTimer(std::chrono::milliseconds duration,
            ...
            if (it == queues_.end()) {
                auto queue = std::unique_ptr<Queue>(new Queue(duration, *this, dispatcher_));
